...
function isWeakEffect(target) {
    ...
    if(item.damage.elementId == -1 && battler.isActor()){
        var rate1 = action.calcNormalAttackElementRate(target,100,false);
        var rate2 = battler.isDualWield() ? action.calcNormalAttackElementRate(target,100,true) : rate1;
        //console.log(rate1,rate2);
        return (rate1+rate2)/2 > 100;
    }
