...
(() => {
  ...
  Window_BattleLog.prototype.addItemNameText = function(item) {
    this._lines.shift();
    if(item.message1 && item.message1 == "runic"){
        var message = BattleManager.runicedMagic().message1;
        if(!BattleManager.runicedMagic().message1){
            message = BattleManager.runicedMagic().name;
        }
        this._lines.push(message);
    }else if(item.message1 && item.message1 != "none" && item.message1 != "throw"){
        this._lines.push(item.message1);
    }else if(item.message1 == "throw"){
        this._lines.push(BattleManager._subject.throwWeapon().name);
    }else if(item.message1 != "none"){
        this._lines.push(item.name);
    }
    this._actionIconIndex = displayIcon ? item.iconIndex : 0;
    if(BattleManager._subject.isCountering()){
        this._actionIconIndex = 64;
    }
    this.refresh();
    // this.wait();
  };
  ...
  Window_BattleLog.prototype.drawLineText = function(index) {
    ...
    this.drawTextEx(text, rect.x, rect.y, Graphics.boxWidth, 'center');
  };
    
    Window_BattleLog.prototype.displayTime = function(){
    ...
    Window_BattleLog.prototype.setDisplayTime = function(value){
        this._displayTime = value;
    }
    
    Window_BattleLog.prototype.updateDisplayTime = function(){
        this._displayTime = this._displayTime > 0 ? this._displayTime - 1 : 0;
    }
