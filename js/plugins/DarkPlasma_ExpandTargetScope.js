...
(() => {
  ...
  Window_BattleEnemy.prototype.processExpandScope = function () {
    if (!$gameParty.inBattle() || (!this instanceof Window_BattleActor && !this instanceof Window_BattleEnemy)) {
      return false;
    }
    const action = BattleManager.inputtingAction();
    if (action && !this.cursorAll() && action.canExpandScope() && Input.isTriggered("pageup") && !this.cursorFixed()) {
    //if (action.canExpandScope() && Input.isTriggered(settings.switchScopeButton) && !this.cursorFixed()) {
      //this.setCursorAll(!this._cursorAll);
    ...
    if (this.cursorAll() && Input.isTriggered("pagedown") && !this.cursorFixed()) {
    //if (action.canExpandScope() && Input.isTriggered(settings.switchScopeButton) && !this.cursorFixed()) {
      //this.setCursorAll(!this._cursorAll);
  ...
  Window_BattleActor.prototype.processExpandScope = function () {
    if (!$gameParty.inBattle() || (!this instanceof Window_BattleActor && !this instanceof Window_BattleEnemy)) {
      return false;
    }
    const action = BattleManager.inputtingAction();
    if (action && !this.cursorAll() && action.canExpandScope() && Input.isTriggered("pagedown") && !this.cursorFixed()) {
    //if (action.canExpandScope() && Input.isTriggered(settings.switchScopeButton) && !this.cursorFixed()) {
      //this.setCursorAll(!this._cursorAll);
      this.setCursorAll(true);
      this.playCursorSound();
    ...
    if (this.cursorAll() && Input.isTriggered("pageup") && !this.cursorFixed()) {
    //if (action.canExpandScope() && Input.isTriggered(settings.switchScopeButton) && !this.cursorFixed()) {
      //this.setCursorAll(!this._cursorAll);
      this.setCursorAll(false);
      this.playCursorSound();
