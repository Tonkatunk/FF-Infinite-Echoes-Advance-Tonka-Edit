    ...
    loadExtendsSymbols(){
       const numExSymbols=this._extendSymbols.length;
        ...
        if(numExSymbols!==this._extendSymbols.length){
            throw new Error("You must not change the number of elements!")
        }
        for (const iterator of this._extendSymbols) {
            iterator.fillSymbol();
        }
...
const KEYS={
    NULL:keyinfo("",0),
    ENTER:keyinfo("Ent",13),
    ENTER_JIS:keyinfo("Ent",13),
    ENTER_JIS_NULL:keyinfo("",13),
    ENTER_NULL:keyinfo("",13),
    ENTER_US:keyinfo("Ent",13),
    SPACE:keyinfo("Space",32),
    SHIFT:keylocked('Sft',16),
    CTRL:keylocked('CTR',17),
    UP:keylocked("↑",38),
    DOWN:keylocked("↓",40),
    LEFT:keylocked("←",37),
    RIGHT:keylocked("→",39),
    ...
    Z:keychar("Z"),
    TAB:keyinfo("TB",9),
    INSERT:keyinfo('In',45),
    BACK:keyinfo('BS',8),
    HOME:keyinfo('HM',36),
    END:keyinfo('ED',35),
    PAGEUP:keyinfo('PU',33),
    PAGEDOWN:keyinfo('PD',34),
    ESC:keyinfo('ES',27),
...
function createErrorTexts(){
    const advanceSymbolInvalid =new MultiLanguageText("The extended symbol must be different than the standard symbol.","拡張シンボルは標準シンボルと異なる内容でなければいけません。");
...
class Scene_GamepadConfig_V8 extends Scene_MenuBaseMVMZ{
    ...
    mainWindowRect(){
        ...
        const height =this.mainWindowHeight()+12
...
function createButtonLayoutChangeCommand(){
    const mText = new MultiLanguageText("Change button notation","表記変更");
